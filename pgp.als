...
pred attacker_can_modify_messages[s,s' : State] {
  attacker_action[s,s'] 
  // <ADD STUFF HERE>
  some m, m' : Message, k, k' : Key, id, id' : Identity | m in s.network and m' in s'.network
			and k in m.contents and k' in m'.contents
			and id in m.addr and id' in m'.addr
     			and ( k != k'  or id != id')
}
...
pred attacker_can_forge_id[s,s' : State] {
  attacker_action[s,s']
  // <ADD STUFF HERE>
  some id, id' : UserId | id not in s.network.addr and id' in s'.network.addr
}
...
pred attacker_can_inject[s,s' : State] {
  attacker_action[s,s'] 
  // <ADD STUFF HERE>
  some m, m' : Message | m not in s.network and m' in s'.network
}
